    ...
    $(function () {
        ...
        */
        ...
        $("#live-feeds__switcher").on("change", function(e) {
            e.preventDefault();
            functionCheckSwitch( $(this), ".switch-content-news"); checkSwitch();
        });
        $("#live-feeds__switcher2").on("change", function(e) {
            e.preventDefault();
            functionCheckSwitch( $(this), ".switch-content-video"); checkSwitch();
        });
        $("#live-feeds__switcher3").on("change", function(e) {
            e.preventDefault();
            functionCheckSwitch( $(this), ".switch-content-photo"); checkSwitch();
        });
        $("#live-feeds__switcher4").on("change", function(e) {
            e.preventDefault();
            functionCheckSwitch( $(this), ".switch-content-history"); checkSwitch();
        });
        ...
        $('.hockey-player-cell__item-name').on('click', function(event) {
          ...
          Foundation.reInit('equalizer');
			...
			checkSwitch();
			//показываем не больше 5-ти элементов живой ленты
			function checkSwitch() {
				var i = 0;
				var prevDate = '';
        $.each($('.switch-content-photo,.switch-content-video,.switch-content-news'),function(index,value){
        	//если чекбокс этого элемента включен, показываем и увеличиваем счеткик, если нет, то прячем
        	if( i<5 && (
        	($(value).attr('class').indexOf('switch-content-photo')>-1 && $('#live-feeds__switcher3').prop('checked'))||
        	($(value).attr('class').indexOf('switch-content-video')>-1 && $('#live-feeds__switcher2').prop('checked'))||
        	($(value).attr('class').indexOf('switch-content-news')>-1 && $('#live-feeds__switcher').prop('checked'))
        	 )){

						var dateDiv = $(value).find("div")[0]; //div с датой
        	 	var curDate = $(dateDiv).children(".live-feeds-news__number").html()+$(dateDiv).children(".live-feeds-news__month").html();
						i++;
						$(value).show();

						//если текущая дата совпадает с предыдущей, не показываем ее
						if (curDate == prevDate){

							 $(dateDiv).removeClass('live-feeds-news__date');
							 $(dateDiv).children().hide();

						}else {
							if($(dateDiv).attr('class').indexOf('live-feeds-news__date')==-1){
								$(dateDiv).addClass('live-feeds-news__date');
								$(dateDiv).children().show();
        ...
        // выподающие меню выравнивание по правому краю последние 2
        $('[data-responsive-menu]').on('show.zf.dropdownmenu',  function(event) {
          var menuLi = $('.menu__li');
          var lastMenuLi = menuLi.length;
          var checkSearchForm = $(this).find('.search-form').length;
          var toLastLiRight;

          if ( checkSearchForm ) {
            toLastLiRight = lastMenuLi - 3;
          } else {
            toLastLiRight = lastMenuLi - 2;
          }

          $('.menu__li').each(function(index) {
            if ( index >= toLastLiRight ) {
              if ( $(this).hasClass('opens-right') ) {
