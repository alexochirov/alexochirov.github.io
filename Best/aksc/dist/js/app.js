...
(function ($) {
  ...
  $(function () {
    ...
    var timer; //**Обработчик кнопки Авторизоваться в модальном окне

    $('#login-content_modal').on('click', '#login_btn_modal', function () {
      var formAuth = $('#form_auth_modal').serialize();
      $.post("/auth/login.php", formAuth, function (data) {
    ...
    $('#reg-login-content').on('click', '#register_btn', function () {
      ...
      $.post("/auth/reg.php?register=yes", formRegister, onAjaxSuccess);
    });
    ...
    $('#reg-login-content').on('click', '#activation_btn', function () {
      ...
      $.post("/auth/reg.php", formActivation, onAjaxSuccess);
    ...
    $('#close_reg_btn').on('click', function () {
      ...
      var regState = $('#regState').attr('value');
      if (regState == 'complete') location.reload();
      setTimeout(function () {
        $('#reg-login-content').html($regLoginContent);
      }, 500);
      clearInterval(timer);
    }); //таймер
    ...
    function initTimer() {
      ...
      var interval_id = setInterval(function () {
        ...
        if (sec != 0) {
          if (sec < 10) $('#clockdiv').html('0' + m + ':0' + sec);else $('#clockdiv').html('0' + m + ':' + sec);
        } else {
          if (m > 0) {
            if (sec < 10) $('#clockdiv').html('0' + m + ':0' + sec);else $('#clockdiv').html('0' + m + ':' + sec);
            m--;
    ...
    var forgotPswContent = $('#forgot-psw-content').html(); //кнопка Закрыть сбрасывает форму восстановления к началу

    $('#close_forgot_btn').on('click', function () {
      setTimeout(function () {
        $('#forgot-psw-content').html(forgotPswContent);
      }, 500);
      clearInterval(timer);
    ...
    $('#forgot-psw').on('click', '#send_activation_code', function () {
      ...
      $.post("/auth/forgot.php", formForgot, function (data) {
    ...
    $('#forgot-psw-content').on('click', '#activation_btn', function () {
      var formActivation = $('#formActivation').serialize() + '&activation_btn=1&curtime=' + $('#clockdiv').html();
      $.post("/auth/forgot.php", formActivation, function (data) {
        $('#forgot-psw-content').html(data);
        var regState = $('#regState').attr('value');
    ...
    $('#forgot-psw-content').on('click', '#change_psw_btn', function () {
      ...
      $.post("/auth/forgot.php", formChangePsw, function (data) {
        $('#forgot-psw-content').html(data);
      });
    });
...
var Favourites = {
  ...
  init: function init(options) {
    ...
    Favourites.delBindings(); //Favourites.delBindingsPage();
  ...
  delBindings: function delBindings() {
  ...
  bindAdd2Favourites: function bindAdd2Favourites() {
    ...
    $(document).on("click", ".js-add2fav", function (e) {
      ...
      favDelTilte = 'Удалить из любимых товаров'; //if (addHtmlSmall) {
      //    favDelHtml = 'Удалить';
      //}
      ...
      $.ajax({
        ...
        if (data.success == 'ADD') {
          ...
          $pThis.addClass('js-del2fav'); //if (addHtml) {
          ...
          var html = data.html; //var countProd = data.countProd;

          Favourites.showPopup(html); //Favourites.updateHeader(countProd);
  ...
  bindDel2Favourites: function bindDel2Favourites() {
    // Удаление из списка
    $(document).on("click", ".js-del2fav", function (e) {
      ...
      var date = new Date(),
          timestamp = date.getTime(),
          $pThis = $(this),
          id = $pThis.data("product-id"); //addHtml = $pThis.data("html");
  ...
  bindDel2FavouritesPage: function bindDel2FavouritesPage() {
    // Удаление из списка
  ...
  // всплывающее окно
  showPopup: function showPopup(html) {
    if (html) {
