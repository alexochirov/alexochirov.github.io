...
Все шрифты попадают в папку *src/fonts/* с любой структурой внутри. Шрифты, из Google Fonts следует подключать в *src/js/app.js* по примеру как сейчас там подключен шрифт Ubuntu. Другие шрифты следует подключать в *src/scss/foundation-settings/_fonts.scss* используя при этом свойство *font-display: swap;*.
...
npm i -g gulp-cli
а затем можно использовать команду (например для страницы shop-product)
gulp --pg shop-product
