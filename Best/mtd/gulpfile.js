...
const paths = {
  ...
  src: {
    ...
    images: 'src/images/**/*.*',
    imagesTiny: 'src/images/**/*.@(png|jpg|jpeg)',
    imagesNoTiny: ['src/images/**/*.*', '!src/images/**/*.@(png|jpg|jpeg)'],
    i: 'src/i/**/*.*',
    fonts: 'src/fonts/**/*.*'
  },
  watch: {
...
let imagesTiny = () => gulp.src(paths.src.imagesTiny, {
    allowEmpty: true
  })
  .pipe(tinypng())
  .pipe(gulp.dest(paths.dist.images));

let imagesNoTiny = () => gulp.src(paths.src.imagesNoTiny, {
  allowEmpty: true
})
  .pipe(gulp.dest(paths.dist.images));
...
gulp.task('minify',
  gulp.series(
    clean,
    gulp.parallel(sass, pug, imagesTiny, imagesNoTiny, i, fonts, jsLib, jsAppMinify)
  )
);
