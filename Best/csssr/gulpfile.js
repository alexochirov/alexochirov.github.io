...
let watch = () => {
  ...
  imagesWatcher.on('unlink', (unlinkPath) => {
    let filePathFromSrc = path.relative(path.resolve('app/images'), unlinkPath);
...
gulp.task('default',
  gulp.series(
    clean,
    gulp.parallel(sass, pug, images, fonts, jsLib, jsApp),
...
gulp.task('minify',
  gulp.series(
    clean,
    gulp.parallel(sass, pug, imagesMinify, fonts, jsLib, jsAppMinify)
  )
);
