...
(function ($, window, document, undefined) {
    ...
    var Orbit = function (el, settings) {
        ...
        self.update_active_link = function (index) {
            ...
            if (settings.thumbnail) {
                var thumb = $('[data-orbit-link="' + index + '"]', thumbnail_container);
                var cont_offset = thumbnail_container.offset().left,
                    thumb_offset = thumb.offset().left,
                    cont_width = thumbnail_container.closest(".orbit-thumbnail-container").outerWidth(),
                    thumb_width = thumb.width();

                if (thumb_offset - cont_offset > cont_width / 2) {
                    var centered_elem = parseInt(settings.thumbnail_count / 2);
                    var max_elem = self.slides().length - settings.thumbnail_count;
                    var max_offset = max_elem * (thumb_width + 15) * -1;
                    var new_offset = (index - centered_elem + 1) * (thumb_width + 15) * -1;
                    if (new_offset >= max_offset)
                        thumbnail_container.animate({'left': new_offset + 'px'}, 500);
                }
                else {
                    thumbnail_container.animate({'left': '0'}, 500);
                }

                thumb.siblings().removeClass(settings.bullets_active_class);
                thumb.addClass(settings.bullets_active_class);
            }
        };
        ...
        self.build_markup = function () {
            ...
            if (settings.thumbnail) {
                thumbnail_container = $('<ol>').addClass(settings.thumbnail_container_class);
                container.append(thumbnail_container);
                thumbnail_container.wrap('<div class="orbit-thumbnail-container"></div>');
                ...
                self.slides().each(function (idx, el) {
                    ...
                    if (img !== undefined) {
                        ...
                        if (src !== undefined && src !== "") {
                            var thumbnail = $('<li><img src="' + src + '" class="orbit-thumbnail-img"/></li> ').attr('data-orbit-link', idx).on('click', self.link_custom);
                            thumbnail_container.append(thumbnail);
                        }
                    }
                });

                $("." + settings.prev_class + ", ." + settings.next_class, container).css({"top": "40%"});
            }
        ...
        self.compute_dimensions = function () {
            ...
            if (settings.thumbnail) {
                container.trigger('ready.fndtn.orbit');
            }
        };
        ...
        self.init = function () {
            ...
            if (settings.thumbnail) {
                container.on('ready.fndtn.orbit', function (e) {
                    setTimeout(function () {
                        var thumb_cont_width = thumbnail_container.closest(".orbit-thumbnail-container").outerWidth(),
                            thumb_margin = 15,
                            thumb_width = ((thumb_cont_width + thumb_margin) / settings.thumbnail_count) - thumb_margin,
                            thumb_height = thumb_width * 0.7;

                        thumbnail_container.closest(".orbit-thumbnail-container").css({"height": thumb_height});

                        $("li", thumbnail_container).css({
                            'width': thumb_width,
                            'height': thumb_height,
                            'margin-right': thumb_margin
                        });
                    }, 1000);
                });
            }
    ...
    Foundation.libs.orbit = {
        ...
        settings: {
            ...
            thumbnail: false,
            thumbnail_container_class: 'orbit-thumbnail',
            thumbnail_count: 6
        },
